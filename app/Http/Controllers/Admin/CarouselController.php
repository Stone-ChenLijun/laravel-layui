...
class CarouselController extends Controller
{
    ...
    {
        return view('admin/index/carousel_list');
    }

    public function addView()
    {
        return view('admin/index/add_carousel');
    }
    ...
    public function editView(Request $request)
    {
        ...
        if (empty($item)) {
            return abort(404, '指定轮播图不存在');
        }
        return view('admin/index/edit_carousel', ['item' => $item]);
    }

    public function list(Request $request)
    {
        $query = Carousel::query()->where('type', Carousel::TYPE_CAROUSEL)->orderByDesc('sort');
    ...
    /**
     * 新建轮播图
     * @Permission(action="admin.carousel.add")
    ...
    public function add(Request $request)
    {
        ...
        $item->type = Carousel::TYPE_CAROUSEL;
    ...
    /**
     * 修改轮播图
     * @Permission(action="admin.carousel.edit")
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     * @throws \Illuminate\Validation\ValidationException
     */
    public function edit(Request $request)
    {
        ...
        $item = Carousel::where('type', Carousel::TYPE_CAROUSEL)->where('id', $request->id)->first();
        if (empty($item)) {
            return $this->respond(-1, '指定轮播图不存在');
        }
        $image = AlbumImage::find($request->image_id);
        if (empty($image)) {
            return $this->respond(-1, '指定图片不存在');
    ...
    /**
     * 删除轮播图
     * @Permission(action="admin.carousel.delete")
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     * @throws \Illuminate\Validation\ValidationException
     */
